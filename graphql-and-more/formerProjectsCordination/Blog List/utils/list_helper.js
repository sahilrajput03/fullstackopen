...
const authorWithMaxBlogs = (bloglist) => {
  //#region comment78
  ...
  //   return maxEl;
  // }
  // const mostblogsauthor = mode(allauthorslist); // get the most occured string
  //#endregion comment78
  const authors = bloglist.map((t) => t.author);
  const desiredAuthor = _.head(_(authors).countBy().entries().maxBy(_.last)); 
  // using lodash, we find author who occured most in the authorList array.
  const numberOfBlogs = authors.filter((t) => t === desiredAuthor).length;
  return { author: desiredAuthor, blogs: numberOfBlogs };
};
